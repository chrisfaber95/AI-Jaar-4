...
def next_spot(dict, key):
    keys = iter(dict)
    if key:
        key in keys  # advance the iterator
    return next(keys, False)


def find_next_cell(grid, cell):
    for next_cell in iter(grid):
        if next_cell and len(grid[next_cell]) > 1:
            return next_cell
    return False

def find_next_cell_ordered(grid):
    smallest_cell = None
    smallest_domain = 10
    for cell, domain in grid.items():
        if len(domain) > 1 and len(domain) < smallest_domain:
            smallest_cell = cell
            smallest_domain = len(domain)
    return smallest_cell

def dfs(grid, cell=None):
    ...
    old_domain = grid[next_cell]
    for digit in grid[next_cell]:
        if no_conflict(grid, next_cell, digit):
            grid[next_cell] = digit
            if dfs(grid, next_cell):
                return True
            grid[next_cell] = old_domain
    return False

def arc_consistency(grid, loc, path):
    if loc == 'I9' :
    ...
    for digit in grid[newpath[-1]]:
        if no_conflict(grid, newpath[-1], str(digit)):
            ...
            updatedgrid = make_arc_consistency(newgrid, newpath[-1], digit)
            temp = next_spot(updatedgrid, loc)
            while len(updatedgrid[temp]) == 1 and temp != 'I9':
                temp = next_spot(updatedgrid, temp)
            if arc_consistency(updatedgrid, temp, newpath):
                return True
    return False

def ac2(grid):
    cell = find_next_cell_ordered(grid)
    if not cell:
        display(grid)
        return True
    
    for digit in grid[cell]:
        if no_conflict(grid, cell, digit):
            new_grid = grid.copy()
            new_grid[cell] = digit
            new_grid = make_arc_consistency(new_grid, cell, digit)
...
def make_arc_consistency(grid, c, v):
    for p in peers[c]:
        if str(v) in grid[p]:
            if len(grid[p]) <= 1:
                return False # conflict
    ...
    for c2 in peers:
        if c2 != c:
            for p2 in peers[c2]:
                if len(grid[p2]) <= 1:
                    if make_arc_consistency(grid, c2, grid[p2]) == False:
                        return grid
    return grid
